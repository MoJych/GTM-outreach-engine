...
def main() -> int:
    ...
    for i, r in enumerate(rows, 1):
        ...
        if passed:
            ...
            # Не прошло гейт — ячейку ОЧИЩАЕМ, а не оставляем как есть.
            # Первая версия «не трогала» ячейку, и при перезапуске в ней
            # оставался вчерашний забракованный текст: защита работала
            # ровно наоборот. Забракованный вариант и причина уходят в
            # Review flag, чтобы ничего не потерялось.
            r[COL_OPENER] = ""
            r[COL_FLAG] = f"ГЕЙТ отклонил: {notes} | текст: {opener}"
            failed += 1
