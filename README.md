...
- **Garbage in, refusal out — and the quality gate caught it live.** Running this against a real batch of ~50 companies, one row's `context` field turned out to be a meta-description ("reference a personalization hook based on a fact") instead of an actual fact, apparently leaked from an upstream Clay AI column that itself failed to find a real signal. Fed that into the LLM, and instead of an opener it wrote a paragraph explaining why it couldn't write one. `passed_quality_check` correctly came back `false` (the length check catches most refusals, since they run long) — confirmed on a real failure, not just in theory. Added an explicit `REFUSAL_PATTERNS` check on top (phrases like "I can't write this", "I'd need real information") so a short refusal can't slip through on word count alone.
